...
describe("tesseraMistralConfig", () => {
  it("returns the Mistral proxy configuration", () => {
    ...
    expect(cfg.defaultHeaders).toEqual({ "x-tessera-api-key": "tsr_test" });
  });
});
...
describe("tesseraConfig (generic dispatcher)", () => {
  it("routes openai", () => {
    const cfg = tesseraConfig("openai", { apiKey: "tsr_test" }) as ReturnType<
  ...
  it("routes anthropic", () => {
    const cfg = tesseraConfig("anthropic", { apiKey: "tsr_test" }) as ReturnType<
...
describe("wrapOpenAI", () => {
  it("overrides baseURL and injects header", () => {
    ...
    const wrapped = wrapOpenAI(base, "tsr_test");
...
describe("wrapAnthropic", () => {
  it("overrides anthropicApiUrl and injects header", () => {
    ...
    >[0];
    const wrapped = wrapAnthropic(base, "tsr_test");
...
describe("wrapMistral", () => {
  it("overrides endpoint and injects header", () => {
    const base = new StubMistralLike() as unknown as Parameters<
      typeof wrapMistral
    >[0];
    const wrapped = wrapMistral(base, "tsr_test");
    const w = wrapped as unknown as StubMistralLike;
    expect(w.endpoint).toBe(`${TESSERA_BASE_URL}/v1/mistral`);
    expect(w.defaultHeaders).toEqual({ "x-tessera-api-key": "tsr_test" });
  });
});
...
describe("wrapGroq", () => {
  it("overrides baseUrl and injects header", () => {
    ...
    const wrapped = wrapGroq(base, "tsr_test");
...
describe("wrapCohere", () => {
  it("overrides baseUrl in clientOptions", () => {
    const base = new StubCohereLike() as unknown as Parameters<
      typeof wrapCohere
    >[0];
    const wrapped = wrapCohere(base, "tsr_test");
